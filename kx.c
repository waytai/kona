...
K vf_ex(V q, K g) 
{
  ...
  SW(t)
  {
    ...
    CS(3, //Executing a {} character function such as {1+1}, {x+y+z-1}, or {[a;b] a+b}
      ...
      if(prnt && f2s && kV(prnt)[LOCALS] && kV(prnt)[CACHE_TREE]){
...
Z K ex2(V*v, K k)  //execute words --- all returns must be Ks. v: word list, k: conjunction?
{
  ...
  if(!v[1] && !k){  // n case
    ...
    if(z>(K)DT_SIZE && z->t==7 && z->n==3){
      if(prnt && f1s && kV(z)[PARAMS] && kV(prnt)[CACHE_TREE] && !kV(z)[CACHE_TREE]){
  ...
  if(bk(v[1])){
    K z= ex_(*v,1);
    if(prnt && z->t==7 && z->n==3 && kV(prnt)[CACHE_TREE] && kV(z)[LOCALS]){
  ...
  if(t3>(K)DT_SIZE && t3->t==7 && t3->n==3){
    if(prnt && kV(prnt)[CACHE_TREE] && kV(prnt)[CACHE_WD]){ f2s=1;
      if(kK(prnt)[CACHE_TREE]->n) {cd(kK(t3)[CACHE_TREE]); kK(t3)[CACHE_TREE]=kK(prnt)[CACHE_TREE]; ci(kK(t3)[CACHE_TREE]);}
    }
    prnt=t3; 
  }
